...
const { getModule, messages, getAllModules, getModuleByDisplayName } = require('powercord/webpack');
...
let channels = await getModule(function(e) {
if(e && e.default && e.default.prototype && e.default.prototype.getHeightForFooter) return true;
})
let NavigableChannels = await getModule(m => m.default && m.default.displayName == 'NavigableChannels');
...
return res / 4;
    ...
    inject('alg-channels-compute', NavigableChannels, 'default', (_, res) => {
      ...
      setTimeout(() => {Array.prototype.forEach.call(document.getElementById('channels').querySelectorAll(`.name-23GUGE.overflow-WK9Ogt`), item => {
      if(!item) return res;
      if(item.fucked) return res;
      item.fucked = true;
      item.setAttribute('data-original', item.innerText);
      try{
      var split = item.innerText.split('-').filter(sp => sp != '').map(s => Array.from(s));
      if(split[0][0].length > 1) item.classList.add('emoji-negative'); else item.classList.remove('emoji-negative');
      var s = '';
      switch(true){
        case split.length == 4:
